...
def fetch_weather(location: str) -> str:
    ...
    if not location or location.strip().lower() in ("current", "here", ""):
        return "Error: Please specify a city or location name for weather lookup."
    ...
    try:
        ...
        req = urllib.request.Request(geo_url, headers={"User-Agent": "Aura-Assistant/1.0"})
        ...
        req_w = urllib.request.Request(weather_url, headers={"User-Agent": "Aura-Assistant/1.0"})
        ...
        logger.error("Weather tool error for %s: %s", location, e)
